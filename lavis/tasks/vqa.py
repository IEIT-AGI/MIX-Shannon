...
class VQATask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, **kwargs):
        ...
        result_file = self.save_result(
            ...
            filename=f"{split_name}_vqa_result_{cur_epoch}",
    ...
    def _report_metrics_bak(self, result_file, split):
        """
    ...
    def _report_metrics(self, result_file, split):
        ...
        metrics = {}
        overall_acc = 0.0
        annos = json.load(open(self.anno_files[split]))
        quesid2annos = dict()
        for ann in annos["annotations"]:
            quesid2annos[ann["question_id"]] = ann

        pred_answers = json.load(open(result_file))
        for t in pred_answers:
            gt_answer = quesid2annos[t["question_id"]]["answers"]
            pred_answer = t["answer"]
            answer_weight = {}
            for answer in gt_answer:
                if answer["answer"] in answer_weight.keys():
                    answer_weight[answer["answer"]] += 1 / len(gt_answer)
                else:
                    answer_weight[answer["answer"]] = 1 / len(gt_answer)

            if pred_answer in answer_weight:
                overall_acc += answer_weight[pred_answer]

        overall_acc = overall_acc / len(pred_answers)
        metrics["agg_metrics"] = overall_acc
        logging.info("Overall Accuracy is: %.02f\n" % overall_acc)
        with open(
                os.path.join(registry.get_path("output_dir"), "evaluate.txt"), "a"
