...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        # if labels is not None and float(loss_weight) > 0:
            if reduction == "mean" and loss_weight is not None:
                loss_fct = CrossEntropyLoss(ignore_index=-100, reduction=reduction)
                # loss = torch.tensor([loss_fct(input.view(-1, input.size(-1)), target.view(-1)) for input, target in zip(lm_logits, labels)], device=loss_weight.device)
                loss = torch.hstack([loss_fct(input.view(-1, input.size(-1)), target.view(-1)) for input, target in zip(lm_logits, labels)])

                loss = loss_weight * loss
                loss = torch.sum(loss)
                loss = loss if loss == 0 else loss / torch.sum(loss_weight)
            else:
                loss_fct = CrossEntropyLoss(ignore_index=-100, reduction=reduction)
