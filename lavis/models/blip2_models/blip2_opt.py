...
class Blip2OPT(Blip2Base):
    ...
    def forward_vqa(self, samples):
        ...
        for b, n in enumerate(samples["n_answers"]):
            ...
            batch_input_tokens_attns += [opt_tokens.attention_mask[b]] * n
            batch_attn_opt += [atts_opt[b]] * n
        ...
        batch_attn_opt = torch.stack(batch_attn_opt, dim=0)
        ...
        attention_mask = torch.cat([batch_attn_opt, batch_input_tokens_attns], dim=1)
        ...
        empty_targets = (
            torch.ones(batch_attn_opt.size(), dtype=torch.long).to(image.device).fill_(-100)
        )
