...
wait
experiment_name=CSI_Task_albert_resnet50
experiment_log_name=${experiment_name}.out
echo "training $experiment_name "
...
CUDA_VISIBLE_DEVICES=0  \
nohup python train.py \
--config-name train_recipe_albert_resnet50  \
trainer.gpus=1 \
seed=666 \
trainer.gradient_clip_val=2 \
trainer.max_epochs=200 \
...
wait
experiment_name=CSI_Task_robeta_resnext101
experiment_log_name=${experiment_name}.out
echo "training $experiment_name "
...
CUDA_VISIBLE_DEVICES=0  \
nohup python train.py \
--config-name train_recipe_robeta_resnext101  \
trainer.gpus=1 \
seed=666 \
trainer.gradient_clip_val=2 \
trainer.max_epochs=200 \
