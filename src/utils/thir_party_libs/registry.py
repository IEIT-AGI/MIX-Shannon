...
def build_from_cfg(cfg: Union[DictConfig, Dict],
                   registry: 'Registry',
    ...
    """
    if not any([isinstance(cfg, dict), isinstance(cfg, DictConfig)]):
    ...
    try:
        is_transforms_cls: Callable = lambda: registry.name in ["text_encoder", "tokenizer"] and hasattr(obj_cls,
                                                                                                         "from_pretrained")
        obj_cls: Callable = getattr(obj_cls, "from_pretrained") if is_transforms_cls() else obj_cls
        return obj_cls(**args)
    ...
    def _register_module(self, module, module_name=None, force=False):
        ...
        for name in module_name:
            if not force and name in self._module_dict:
                logger.warning(f'{name} is already registered in {self.name}')
                continue
