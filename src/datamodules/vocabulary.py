...
class Vocab(object):
    ...
    def __init__(self, vocab_file, max_size):
        ...
        with open(vocab_file, 'r', encoding='utf8') as vocab_f:  # New : add the utf8 encoding to prevent error
            ...
            for line in vocab_f:
                ...
                if w in self._word_to_id:
                    ...
                    print(f"Duplicated word in vocabulary file Line {cnt} : {w}")
                    continue
                self._word_to_id[w] = self._count
                self._id_to_word[self._count] = w
                self._count += 1
        ...
        print(
            f"Finished constructing vocabulary of {self._count} total words. Last word added: {self._id_to_word[self._count - 1]}")
