...
def hard_example_mining(dist_mat, labels, return_inds=False, mask_for_multimodal=False):
...
class TripletLoss(object):
    ...
    def __init__(self, margin=1.5, mask_for_multimodal=False):
        self.margin = margin
        if margin is not None:
            self.ranking_loss = nn.MarginRankingLoss(margin=margin)
        else:
            self.ranking_loss = nn.SoftMarginLoss()
        self.mask_for_multimodal = mask_for_multimodal

    def __call__(self, global_feat, labels, normalize_feature=False):
        if normalize_feature:
            global_feat = normalize(global_feat, axis=-1)
        dist_mat = euclidean_dist(global_feat, global_feat)
        dist_ap, dist_an = hard_example_mining(dist_mat, labels, mask_for_multimodal=self.mask_for_multimodal)
